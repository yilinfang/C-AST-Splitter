				...
				if (args[i].length() > 3) {
					...
					switch (opt) {
						...
						case "help":
							printHelp(null);
							break;
						// Nodes are made up of "block" instead of "statement"
//						case "node_level":
//							exec.addAnalysisOption(Execution.Analysis.NODE_LEVEL);
//							break;
						//
						case "outdir":
							if (i < args.length - 1) {
						...
						case "node_level":
							if (i < args.length - 1) {
								...
								switch (args[i].toLowerCase()) {
									case "statement":
										exec.setNodeLevel(Execution.NodeLevels.statement);
										break;
									case "block":
										exec.setNodeLevel(Execution.NodeLevels.block);
										break;
									default:
										printHelp("Unknown node level: " + args[i]);
										System.exit(1);
								}
							} else {
								printHelp("Format not specified!");
						...
						case "lang":
							if (i < args.length - 1) {
								...
								switch (args[i].toLowerCase()) {
									case "c":
										exec.setLanguage(Execution.Languages.C);
										break;
									case "java":
	...
	public static void printHelp(String errMsg) {
		...
		String[] help = {
			...
			"   -node_level Specify Nodes level; eithrt 'block' or 'statement'",
